...
Formula for calculating the $nth$ derivative of $y=1/x=x^{-1}$ :
$$
y^{(n)} = (-1)^nn!x^{-(n+1)}
$$
...
Into the second derivative function we can plug in $\frac{1}{2}$ for time, so 
$$
